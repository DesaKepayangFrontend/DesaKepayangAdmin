...
const SearchPendudukPage = () => {
    ...
    // Fetch data RTRW untuk dropdown
    const fetchRTRW = useCallback(async () => {
        try {
    ...
    // Fungsi untuk handle gender change
    ...
    // Fungsi untuk handle submit (edit)
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const url = `https://desakepayangbackend-production.up.railway.app/penduduk/${currentPenduduk?.id_penduduk}`;
            const method = 'PUT';
            ...
            setShowModal(false);
            // Refresh hasil pencarian setelah edit
            fetchSearchResults(searchTerm);
        } catch (err) {
            console.error(err);
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            ...
            // Refresh hasil pencarian setelah hapus
            fetchSearchResults(searchTerm);
